...
def nfa_to_matrix(automaton: NondeterministicFiniteAutomaton) -> FiniteAutomaton:
    ...
    m = {label: dok_matrix((len_states, len_states), dtype=bool) for label in automaton.symbols}

    for u, edges in states.items():
        for label, v_set in {k: [v] if not isinstance(v, set) else list(v) for k, v in edges.items()}.items():
            for v in v_set:
        ...
        for u in range(m_size):
            for v in range(m_size):
                if automaton.m[label][u, v]:
                    nfa.add_transition(
                        mapping_reverse[u], label, mapping_reverse[v]
                    )
...
def intersect_automata(
    ...
    m = {label: kron(automaton1.m[label], automaton2.m[label], "csr") for label in
         automaton1.m.keys() & automaton2.m.keys()}
    start = {State(len(automaton2.mapping) * automaton1.mapping[u] + automaton2.mapping[v]) for u, v in
             product(automaton1.start, automaton2.start)}
    final = {State(len(automaton2.mapping) * automaton1.mapping[u] + automaton2.mapping[v]) for u, v in
             product(automaton1.final, automaton2.final)}
    mapping = {len(automaton2.mapping) * i + j: len(automaton2.mapping) * i + j for i in automaton1.mapping.values() for
               j in automaton2.mapping.values()}
